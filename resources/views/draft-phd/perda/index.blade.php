...
@section('content')
                            ...
                            <ul class="dropdown-menu" aria-labelledby="sortDropdown">
                                <li>
                                    <a class="dropdown-item {{ !isset($sortOrder) || $sortOrder == 'desc' ? 'active bg-primary text-white' : '' }}"
                                        href="{{ route('draft-phd.perda.index', array_merge(request()->query(), ['sort' => 'desc'])) }}">
                                        Terbaru ke Terlama
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item {{ isset($sortOrder) && $sortOrder == 'asc' ? 'active bg-primary text-white' : '' }}"
                                        href="{{ route('draft-phd.perda.index', array_merge(request()->query(), ['sort' => 'asc'])) }}">
                                        Terlama ke Terbaru
                                    </a>
                                </li>
                            </ul>
                            ...
                            @forelse($perdas as $index => $perda)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        {{ $index + 1 + ($perdas->currentPage() - 1) * $perdas->perPage() }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        {{ $perda->no_agenda }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        {{ $perda->no_surat }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        {{ $perda->pengirim }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        {{ $perda->tanggal_terima->format('d/m/Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        @if ($perda->disposisi)
                                            @php
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                                ...
                                                @if (auth()->user()->role !== 'monitor')
                                                    ...
                                                    <li>
                                                        <button type="button" class="dropdown-item text-danger"
                                                            onclick="confirmDelete({{ $perda->id }})">
                                        ...
                                        <form id="delete-form-{{ $perda->id }}"
                                            action="{{ route('draft-phd.perda.destroy', $perda->id) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                ...
                <form id="disposisiForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            @else
                                <span id="statusPersetujuanPerda" class="badge bg-secondary"></span>
                            @endif
                            <input type="hidden" id="disposisiPerdaId" name="disposisiPerdaId" />
                        </div>
                        <div class="mb-3">
    ...
    <script>
        ...
        function openDisposisiModal(id, buttonElement) {
            // Validasi parameter
            if (!id) {
                console.error('Perda - ID tidak valid');
                return;
            }

            const form = document.getElementById('disposisiForm');
            if (!form) {
                console.error('Perda - Form tidak ditemukan');
                return;
            }

            form.action = `/draft-phd/perda/${id}/disposisi`;
            const suratIdInput = document.getElementById('disposisiPerdaId');
            if (suratIdInput) {
            ...
            console.log('Perda - Data from button attributes:', {
                persetujuan,
                tujuan,
                subDisposisi,
                tanggal,
            ...
            console.log('Perda - Setting persetujuan:', persetujuanValue, '->', status);
            ...
            if (isAdmin) {
                ...
                if (radioDisetujui && radioBelum) {
                    if (persetujuanValue === 'Sudah') {
                        ...
                        console.log('Perda - Radio Sudah checked');
                    } else {
                        radioDisetujui.checked = false;
                        radioBelum.checked = true;
                        console.log('Perda - Radio Belum checked');
                    }
                }
            } else {
                const statusEl = document.getElementById('statusPersetujuanPerda');
                if (statusEl) {
            ...
            if (tujuan) {
                ...
                if (disposisiSelect) {
                    ...
                    console.log('Perda - Setting tujuan disposisi:', tujuanValue);
                    ...
                    for (let i = 0; i < disposisiSelect.options.length; i++) {
                        ...
                        if (option.value === tujuanValue || option.textContent.trim() === tujuanValue) {
                            ...
                            found = true;
                            console.log('Perda - Found matching option:', option.value);
                            break;
                        }
                    }
                    ...
                    if (!found) {
                        for (let i = 0; i < disposisiSelect.options.length; i++) {
                            ...
                            if (option.value.includes(tujuanValue) || tujuanValue.includes(option.value) ||
                                ...
                                disposisiSelect.value = option.value;
                                found = true;
                                console.log('Perda - Found partial matching option:', option.value);
                                break;
                            }
                        }
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if (selectedTujuan !== 'Kasubag Tata Usaha' && subDisposisi && subDisposisi.trim()) {
                            setTimeout(() => {
                                ...
                                console.log('Perda - Setting sub_disposisi:', subDisposisi);
                                ...
                                if (subDisposisiContainer && subDisposisiSelect) {
                                    ...
                                    for (let i = 0; i < subDisposisiSelect.options.length; i++) {
                                        ...
                                        if (option.value === subDisposisiValue ||
                                            ...
                                            found = true;
                                            console.log('Perda - Found matching sub_disposisi option:',
                                                option.value);
                                            break;
                                        }
                                    }
                                    ...
                                    if (!found && subDisposisiValue) {
                                        ...
                                        console.log('Perda - Added new sub_disposisi option:',
                                            subDisposisiValue);
                                    }
            ...
            if (tanggalInput) {
                if (tanggal && tanggal.trim()) {
                    ...
                    console.log('Perda - Setting tanggal disposisi from data:', tanggalValue);

                    // Jika format YYYY-MM-DD sudah benar, gunakan langsung
                    if (tanggalValue.match(/^\d{4}-\d{2}-\d{2}$/)) {
                        tanggalInput.value = tanggalValue;
                        console.log('Perda - Tanggal set to (YYYY-MM-DD):', tanggalValue);
                    } else {
                        ...
                        if (dateParts.length === 3) {
                            ...
                            console.log('Perda - Converted tanggal:', tanggalInput.value);
                        } else {
                            console.log('Perda - Invalid tanggal format, using default');
                            setDefaultDate();
                        }
                    }
                } else {
                    console.log('Perda - No tanggal data, using default');
                    setDefaultDate();
                }
            }
            ...
            setTimeout(() => {
                ...
                if (catatanInput) {
                    if (catatan && catatan.trim()) {
                        ...
                        console.log('Perda - Setting catatan:', catatan.trim());
                    } else {
                        catatanInput.value = '';
                        console.log('Perda - No catatan data, clearing field');
                    }
                } else {
                    console.error('Perda - Catatan input element not found');
                }
            }, 250);
            ...
            setTimeout(() => {
                ...
                if (modalElement) {
                    ...
                    setTimeout(() => {
                        console.log('Perda - Modal shown, final values:');
                        console.log('Perda - Disposisi:', document.getElementById('disposisi')?.value);
                        console.log('Perda - Tanggal:', document.getElementById('tanggal_disposisi')
                        ?.value);
                        console.log('Perda - Catatan:', document.getElementById('catatan')?.value);
                    }, 100);
                }
            }, 100);
        }
        ...
        if (document.getElementById('radioPersetujuanGroup')) {
            document.getElementById('radioPersetujuanGroup').addEventListener('change', function(e) {
                const id = document.getElementById('disposisiPerdaId').value;
                fetch(`/api/perda/${id}`)
                    .then(res => res.json())
                    .then(data => {
                        ...
                        sendUpdatePersetujuanPerda(id, disposisiBaru, statusSetuju);
                    });
            });
        }

        function sendUpdatePersetujuanPerda(id, disposisiBaru, statusSetuju) {
            fetch(`/draft-phd/perda/${id}/update-disposisi`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const statusPersetujuanEl = document.getElementById('statusPersetujuanPerda');
